...
void USART1_IRQHandler(void)
{
    if((USART1->SR & USART_FLAG_RXNE) != (uint16_t)RESET)
	{
        ...
        if (come_data == 0xa5&&UsartRx2.DLC == 0)  /* 如果遇到起始字节 */
        {
    ...
    switch(pData[5])
	{
		...
		case 2:
			if(is_pos)
			{
				...
				{
					dir_angle_wheel[0] = -(motor_pos[2] - motor1_value_begining)*90.0/4000.0;
				}
				else if(node == 4 && init_success == 1)
				{
					dir_angle_wheel[1] = -(motor_pos[3] - motor2_value_begining)*90.0/4000.0;
				}
			}
			else
			{
