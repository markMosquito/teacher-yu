...
{    
	...
	if((tmp[0]+tmp[1]+tmp[2]+tmp[3]) == tmp[4])
	{
		...
		//控制速度
		if(remote_data[1] < 1000)
		{
			speed = (remote_data[1] - 1000)*100;
		}
		else if(remote_data[1] > 3000)
		{
			speed = (remote_data[1] - 3000)*100;
		}
		else
		{
			speed = 0;
		...
		//控制旋转
		if(remote_data[2] < 1500)
		{
			rotating_speed = (remote_data[2] - 1500)*100;
			rotating_flag = 1;
		}
		else if(remote_data[2] > 2500)
		{
			rotating_speed = (remote_data[2] - 2500)*100;
			rotating_flag = 1;
		}
		else
		{
    ...
    switch(pData[5])
	{
		...
		case 2:
			if(is_pos)
			{
				...
				{
					dir_angle_wheel[0] = -(motor_pos[2] - motor1_value_begining)*90.0/4000.0 - 2.5;
				}
